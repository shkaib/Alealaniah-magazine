        ...
        <div class="col-lg-6 col-md-6 colsm-12 col-xs-12 nogape pull-right detailBox" id="scroller" style="padding: 100px 60px; background: #efefef; overflow: hidden; color: #939694;">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    ...
                    <div class="col-lg-10 nogape">
                        <h2 class="font-light">
                            <a href="{{ URL::route( 'companies' , [ $customer->user_id ] ) }}">
                                {{ $customer->name }}
                            </a>
                        </h2></div>
                    <div class="col-lg-2 nogape text-right"><h3 class="color-lighGreen">{{ $customer->price }}</h3></div>
                    <!-- /Heading -->
                    
                    <!-- Featured Image Area -->
                    <div class="col-lg-12 nogape">
                        @if( !empty( $customer->logo ) )
                            <img class="full" src="{{ '/public/'.$customer->logo  }}" />
                        @else
                    ...
                    <div style="background: #C1C1C1;" class="address col-lg-12 nogape padding-15 margin-top-5 margin-bottom-10">
                        ...
                        <div class="col-lg-11  col-md-11 col-sm-11 col-xs-11"><?php echo $customer->address; ?></div>
                    </div>
                    <!-- /Address -->
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 contactEmail">
                    ...
                    @if( $customer->email != '' || $customer->phone != '' )
                        @if( $customer->phone != '' )
                            <div class="full font-12 margin-top-5 margin-bottom-5 pull-left"><span><i class="fa fa-phone" aria-hidden="true"></i></span> <span>{{ $customer->phone }}</span></div>
                        @endif
                    
                        @if( $customer->email != '' )
                        <div class="full font-12 margin-top-5 margin-bottom-5 pull-left"><span><i class="fa fa-envelope-o" aria-hidden="true"></i></span> <span>{{ $customer->email }}</span></div>
                        @endif
                    @else
                    
                    
                    <div class="contact-details">
                        <div class="full font-12 margin-top-5 margin-bottom-5 pull-left"><span><i class="fa fa-phone" aria-hidden="true"></i></span> <span>{{ $customer->getCustomerNumber() }}</span></div>
                        <div class="full font-12 margin-top-5 margin-bottom-5 pull-left"><span><i class="fa fa-envelope-o" aria-hidden="true"></i></span> <span>{{ $customer->getCustomerEmail() }}</span></div>
                        @if( !empty($customer->getCustomerFacebook()) )
                            <div class="full font-12 margin-top-5 margin-bottom-5 pull-left"><span><i class="fa fa-facebook" aria-hidden="true"></i></span> <span>{{ $customer->getCustomerFacebook() }}</span></div>
                        @endif
                        
                        @if( !empty( $customer->getCustomerInstagram() )  )
                            <div class="full font-12 margin-top-5 margin-bottom-5 pull-left"><span><i class="fa fa-instagram" aria-hidden="true"></i></span> <span>{{ $customer->getCustomerInstagram() }}</span></div>
                        @endif
                        
                        @if( !empty( $customer->getCustomerTwitter() )  )
                            <div class="full font-12 margin-top-5 margin-bottom-5 pull-left"><span><i class="fa fa-twitter" aria-hidden="true"></i></span> <span>{{ $customer->getCustomerTwitter() }}</span></div>
                        @endif
                    </div>
                    @endif
                    ...
                    <div class="contact-form margin-top-5">
                        ...
                        <form method="post" action="{{ URL::route('sendToCustomer', [$customer->id]) }}">
                            @if( $customer->email != '' )
                                <input type="hidden" class="form-control" name="customer-email" value="{{ $customer->email }}" />
                            @else
                                <input type="hidden" class="form-control" name="customer-email" value="{{ $customer->getCustomerEmail()  }}" />
                            @endif
                    ...
                    <!--<div class="border-1 padding-15 margin-top-10">
                        ...
                        <div class="row nogape margin-bottom-5">
                            <div class="col-lg-6">Location</div><div class="col-lg-6 text-right"><?php echo  $customer->address ?></div>
                        </div>
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 shareComments">
                    ...
                    <div class="fb-comments" data-href="http://alealaniah.com/single-ad/{{ $customer->id }}" data-width="100%" data-numposts="5"></div>
                </div>
...
function initMap() {
  var myLatLng = {lat: {{ $customer->map_lat }}, lng: {{ $customer->map_lng }} };
          ...
          navigator.geolocation.getCurrentPosition(function(location) {
            var myLatLng = {lat: location.coords.latitude, lng: location.coords.longitude};
                ...
                directionsService.route({
                  ...
                  destination: {lat: {{ $customer->map_lat }}, lng: {{ $customer->map_lng }}},  // Ocean Beach.
